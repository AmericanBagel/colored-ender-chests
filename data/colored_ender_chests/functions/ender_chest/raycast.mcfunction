...
execute at @e[type=minecraft:marker,tag=iris.ray] align xyz positioned ~.5 ~ ~.5 if block ~ ~ ~ minecraft:barrel if entity @e[tag=colored_ender_chests.ender_chest.marker,distance=..1] run function colored_ender_chests:__private__/anonymous/3